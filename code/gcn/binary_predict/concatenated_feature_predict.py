...
def main(args):
    ...
    for i in range(fold):
        train_data, test_data, num_nodes, num_rels, node_features_we = train_test_triples(path, i, 'embedding',
                                                                                           args.n_hidden)
        _, _, _, _, node_features_pca = train_test_triples(path, i, 'analogy', args.n_hidden)
        ...
        model_state_file = args.dataset + '_concatenated_model_state.pth'
    ...
    results_out += "Mean values over " + str(
            fold) + " fold: Precision: {:.4f} | Recall: {:.4f} | F1: {:.4f}".format(
...
if __name__ == '__main__':
    ...
    parser.add_argument("--graph-batch-size", type=int, default=500,
