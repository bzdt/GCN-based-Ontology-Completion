...
def main(args):
    ...
    for i in range(fold):
        ...
        _, _, _, _, _, _, _, _, _, _, _, node_features_pca = train_test_idx(path, i, 'analogy', args.n_hidden)
        node_features = np.hstack((np.array(node_features_we), np.array(node_features_pca)))  # concatenated features
        ...
        pred_rules, str_prerule = find_rules_ut(path, i, test_idx, tmp_label, labels_test_pre, test_template_id,
                                                    pred=True)
        ...
        print("Test Precision: {:.4f} | Test Recall: {:.4f} | Test F1: {:.4f}".format(test_precision, test_recall,
                                                                                          test_f1))
    ...
    print("Mean values over " + str(fold) + " fold: Precision: {:.4f} | Recall: {:.4f} | F1: {:.4f}".format(mean_p,
                                                                                                                mean_r,
                                                                                                                mean_f1))
    output += "Mean values over " + str(fold) + " fold: Precision: {:.4f} | Recall: {:.4f} | F1: {:.4f}\n".format(
            mean_p, mean_r, mean_f1)

    file_name = 'concatenated_' + args.dataset + '_' + str(args.n_hidden) + '.txt'
...
if __name__ == '__main__':
    ...
    parser.add_argument("-d", "--dataset", type=str, default='sumo',
                       help="dataset to use")
